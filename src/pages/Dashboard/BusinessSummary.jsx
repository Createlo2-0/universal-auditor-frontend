...
const generatePDF = () => {
    ...
    // Draw container for all content
    ...
    // Main background box for overview
    ...
    // Add border to Company Overview section
    ...
    // Add border to the circular chart section
    doc.setLineWidth(0.5);
    doc.setDrawColor(...colors.border);
    ...
    // Create progress arc with gradient from blue to light blue
    for (let angle = startAngle; angle < endAngle; angle += 1) {
        const progress = (angle - startAngle) / totalAngleRange;
        
        // Interpolate between blue and light blue
    ...
    const drawInstagramIcon = (x, y, size) => {
        ...
        // Inner camera frame border
        doc.setLineWidth(size * 0.025 * scaleFactor);
        const frameInset = size * 0.03;
    ...
    const drawWebsiteIcon = (x, y, size) => {
        ...
        // Latitude lines (horizontal curved lines)
    ...
    doc.roundedRect(cardX + shadowOffset, y + shadowOffset, cardWidth, cardHeight, 4, 4, "F");
    ...
    const drawSecondPage = (doc, colors, reportData, pageWidth, pageHeight) => {
        ...
        // Convert large paragraph into exactly 3 bullet points
        ...
        const summaryPoints = processBulletData(reportData.businesssummary, true);
        y = drawBulletPoints(summaryPoints, y) + 8; // Reduced from 15 to 8

        // INSIGHTS Section
        y = drawSectionHeader("KEY INSIGHTS", y, "insights");
        const insightPoints = processBulletData(reportData.insights);
        y = drawBulletPoints(insightPoints, y, 5) + 8; // Reduced from 15 to 8
